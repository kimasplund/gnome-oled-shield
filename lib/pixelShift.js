...
var PixelShift = class PixelShift {
    ...
    _calculateNextShift() {
        // Pattern: move in a 1-pixel square
        let x = this._currentShift.x;
        let y = this._currentShift.y;

        if (x === 0 && y === 0) return { x: PIXEL_SHIFT_AMOUNT, y: 0 };
        if (x === PIXEL_SHIFT_AMOUNT && y === 0) return { x: PIXEL_SHIFT_AMOUNT, y: PIXEL_SHIFT_AMOUNT };
        if (x === PIXEL_SHIFT_AMOUNT && y === PIXEL_SHIFT_AMOUNT) return { x: 0, y: PIXEL_SHIFT_AMOUNT };
        return { x: 0, y: 0 }; // Complete the square
    }
