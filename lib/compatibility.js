...
/**
 * Add logical assignment operators polyfill
 * @param {object} target - Object to add polyfills to
 */
function addLogicalAssignmentPolyfills(target) {
    // Add the polyfills to Object.prototype
...
// Conditionally add logical assignment polyfills
if (!hasNativeLogicalAssignment) {
    // We only add the helper methods to global scope if needed
    // These can be used with obj.nullishAssign('prop', value) instead of obj.prop ??= value
    addLogicalAssignmentPolyfills(globalThis);
}
