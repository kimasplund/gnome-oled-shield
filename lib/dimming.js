...
export const Dimming = GObject.registerClass(
    class Dimming extends GObject.Object {
        ...
        _validateSettings() {
            const requiredBooleanKeys = ['debug-mode', 'dimming-enabled'];
            const requiredIntKeys = ['dimming-brightness', 'dimming-interval'];
        ...
        applyDimming() {
            if (!this._settings.get_boolean('dimming-enabled')) {
                this._log('Dimming is disabled');
                return;
            }

            const brightness = this._settings.get_int('dimming-brightness') / 100;
