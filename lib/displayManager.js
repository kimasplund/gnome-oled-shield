...
export default class DisplayManager extends EventEmitter {
    ...
    #applyMonitorSettings(monitor, settings) {
        try {
            ...
            // This would use the appropriate API for the monitor type
    ...
    #getMonitorId(monitor) {
        try {
            // Try to get a unique ID based on connector and manufacturer
            const connector = monitor.get_connector ? monitor.get_connector() : null;
            const manufacturer = monitor.get_manufacturer ? monitor.get_manufacturer() : null;
            const model = monitor.get_model ? monitor.get_model() : null;
            const displayName = monitor.get_display_name ? monitor.get_display_name() : null;
            
            // Build a unique ID from available properties
            const idParts = [
                manufacturer,
                model,
                connector,
            ...
            // Fallback to index if no identifiable information
            return `monitor-${this.#monitors.indexOf(monitor)}`;
        } catch (error) {
            this.#debug(`Error getting monitor ID: ${error.message}`);
            return `monitor-${Math.random().toString(36).substring(2, 9)}`;
        }
    }
    ...
    #detectDisplayType(monitor) {
        try {
            ...
            const model = monitor.get_model ? monitor.get_model() : '';
            const manufacturer = monitor.get_manufacturer ? monitor.get_manufacturer() : '';
            const displayName = monitor.get_display_name ? monitor.get_display_name() : '';
    ...
    async refresh() {
        ...
        try {
            ...
            this.#monitors = this.#monitorManager.get_monitors?.() || [];
            this.#primaryMonitor = this.#monitorManager.get_primary_monitor?.() || null;
    ...
    getDisplayInfo() {
        return this.#monitors.map(monitor => {
            ...
            return {
                id: monitorId,
                name: monitor.get_display_name?.() || 'Unknown',
