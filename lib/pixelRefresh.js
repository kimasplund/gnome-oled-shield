...
export default class PixelRefresh extends EventEmitter {
    ...
    #syncStatusToSettings(running, progress) {
        ...
        try {
            ...
            if (running && progress > 0) {
                const elapsedUs = GLib.get_monotonic_time() - this.#refreshStartTime;
                ...
                this.#settings.set_int(CONSTANTS.SETTINGS.PIXEL_REFRESH_TIME_REMAINING, remaining);
            } else {
    ...
    #runRefreshAnimation(startProgress) {
        ...
        const animateFrame = () => {
            ...
            const prevPct = Math.floor(this.#progress * 20); // 5% buckets
            this.#progress = Math.min(1.0, this.#progress + 0.01);
            const curPct = Math.floor(this.#progress * 20);
            ...
            // Sync to GSettings every 5% to keep prefs UI updated without excessive writes
            if (curPct !== prevPct) {
